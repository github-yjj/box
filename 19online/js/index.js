...
class Show_sleft{
  ...
  init() {
    this.cs3_r_gds.addEventListener("click",shTip);
    function shTip(e){
      var tgt=[];
  ...
  display() {
    var str = "";
    for(var i=0;i<4;i++){
      str+=`<li idenf="${this.res[i].udid}">
      <a href="http://localhost/19online/detail.html"><img src="${this.res[i].url}" alt=""></a>
      <span>${this.res[i].name}</span>
      <span>零售价：<i>￥${this.res[i].price}</i></span>
      <span>会员价：<b>￥${this.res[i].wprice}</b></span>
  </li>
...
class InfoLog{
  constructor(){
    this.loginfo=document.querySelectorAll(".info-spei");
  ...
  init(){
    if(localStorage.getItem("logname")!==''){
      this.loginfo[0].innerHTML='hi,Tom';
      this.loginfo[1].innerHTML="退出"
    }else{
      this.loginfo[0].innerHTML="注册";
      this.loginfo[1].innerHTML="登录"
    }
